...
Correct mapping:
- ALG=0: Naive (despite "adv rel err" label)
- ALG=2: ADV
- ALG=3: ADV+
- ALG=4: ADV++
"""
...
def parse_lrcwiki_2M_file():
    """Parse the p3_lrcwiki_2M.txt file and extract results"""
    ...
    for i in range(1, len(sections), 3):
        ...
        # Extract relative error - use "relative error =" for ALG=0, "adv rel err =" for others
        if alg_num == 0:
            rel_err_match = re.search(r'relative error = ([\d.e+-]+)', section_content)
        else:
        ...
        # Extract estimate
        if alg_num == 0:
            # For Naive, look for "estimate ="
            estimate_match = re.search(r'estimate = ([\d.e+-]+)', section_content)
        else:
            # For ADV algorithms, look for "estimate ="
            estimate_match = re.search(r'estimate = ([\d.e+-]+)', section_content)
