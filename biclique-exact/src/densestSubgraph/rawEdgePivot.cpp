...
void rawEdgePivot::localCount() {
    // printf("[localCount] p=%u q=%u n1=%u n2=%u m=%u\n", p, q, g->n1, g->n2, g->m);fflush(stdout);
    // Fast path: p == 1 — each upper vertex forms C(deg,q) bicliques
    if(p == 1) {
        for(uint32_t u = 0; u < g->n1; u++) {
            uint32_t du = g->deg1(u);
            if(du >= q) {
                double c = C[du][q];
                (*localAns)[u] += c;
                ans += c;
            ...
            for(uint32_t i = g->pU[u]; i < g->pU[u + 1]; i++) {
                uint32_t v = g->e1[i];
                if(du >= q)
                    (*localAns)[v + g->n1] += C[du - 1][q - 1];
            }
        }
        return;
    }
    // Fast path: q == 1 — each lower vertex forms C(deg,p) bicliques
    if(q == 1) {
        for(uint32_t v = 0; v < g->n2; v++) {
            uint32_t dv = g->deg2(v);
            if(dv >= p) {
                double c = C[dv][p];
                (*localAns)[v + g->n1] += c;
                ans += c;
            ...
            for(uint32_t i = g->pV[v]; i < g->pV[v + 1]; i++) {
                uint32_t u = g->e2[i];
                if(dv >= p)
                    (*localAns)[u] += C[dv - 1][p - 1];
            }
        }
        return;
    }
    // Fast path for p == 2 or q == 2: direct pair counting
    // For (2,q): iterate lower side, count upper-vertex pairs — cost O(sum_v C(deg2(v),2))
    // For (p,2): iterate upper side, count lower-vertex pairs — cost O(sum_u C(deg1(u),2))
    // For (2,2): pick whichever side has smaller pair cost
    if(p == 2 || q == 2) {
        double costLower = 0, costUpper = 0;
        bool useLowerSide;
        if(p == 2 && q == 2) {
            for(uint32_t v = 0; v < g->n2; v++) {
                double d = g->deg2(v);
            ...
            useLowerSide = (costLower <= costUpper);
        } else {
            useLowerSide = (p == 2);
        }

        if(useLowerSide) {
            // p == 2 path: iterate lower vertices, count pairs of upper neighbors
            uint32_t qq = q;
            std::unordered_map<uint64_t, uint32_t> pair_cnt;
            pair_cnt.reserve(g->m);
            for(uint32_t v = 0; v < g->n2; v++) {
                for(uint32_t i = g->pV[v]; i < g->pV[v + 1]; i++) {
                    uint32_t u1 = g->e2[i];
                    for(uint32_t j = i + 1; j < g->pV[v + 1]; j++) {
                        uint32_t u2 = g->e2[j];
                        pair_cnt[((uint64_t)u1 << 32) | u2]++;
                    }
                }
            }
            for(auto& kv : pair_cnt) {
                uint32_t c = kv.second;
                if(c < qq) continue;
                uint32_t u1 = (uint32_t)(kv.first >> 32);
                uint32_t u2 = (uint32_t)(kv.first & 0xFFFFFFFF);
                double bc = C[c][qq];
                ans += bc;
                (*localAns)[u1] += bc;
                (*localAns)[u2] += bc;
            ...
            for(uint32_t v = 0; v < g->n2; v++) {
                for(uint32_t i = g->pV[v]; i < g->pV[v + 1]; i++) {
                    ...
                    for(uint32_t j = i + 1; j < g->pV[v + 1]; j++) {
                        uint32_t u2 = g->e2[j];
                        uint32_t c = pair_cnt[((uint64_t)u1 << 32) | u2];
                        if(c >= qq)
                            (*localAns)[v + g->n1] += C[c - 1][qq - 1];
                    }
                }
            }
        } else {
            // q == 2 path: iterate upper vertices, count pairs of lower neighbors
            uint32_t pp = p;
            std::unordered_map<uint64_t, uint32_t> pair_cnt;
            pair_cnt.reserve(g->m);
            for(uint32_t u = 0; u < g->n1; u++) {
                for(uint32_t i = g->pU[u]; i < g->pU[u + 1]; i++) {
                    uint32_t v1 = g->e1[i];
                    for(uint32_t j = i + 1; j < g->pU[u + 1]; j++) {
                        uint32_t v2 = g->e1[j];
                        pair_cnt[((uint64_t)v1 << 32) | v2]++;
                    }
                }
            }
            for(auto& kv : pair_cnt) {
                uint32_t c = kv.second;
                if(c < pp) continue;
                uint32_t v1 = (uint32_t)(kv.first >> 32);
                uint32_t v2 = (uint32_t)(kv.first & 0xFFFFFFFF);
                double bc = C[c][pp];
                ans += bc;
                (*localAns)[v1 + g->n1] += bc;
                (*localAns)[v2 + g->n1] += bc;
            ...
            for(uint32_t u = 0; u < g->n1; u++) {
                for(uint32_t i = g->pU[u]; i < g->pU[u + 1]; i++) {
                    ...
                    for(uint32_t j = i + 1; j < g->pU[u + 1]; j++) {
                        uint32_t v2 = g->e1[j];
                        uint32_t c = pair_cnt[((uint64_t)v1 << 32) | v2];
                        if(c >= pp)
                            (*localAns)[u] += C[c - 1][pp - 1];
                    }
                }
            }
        }
    ...
    // Generic recursive counting for (p,q) with p>=3 and q>=3
    for(uint32_t u = 0; u < g->n1; u++) {
        int pR = 0;
        for(uint32_t i = g->pU[u]; i < g->pU[u + 1]; i++) {
            uint32_t v = g->e1[i];
